	...
	try{
		...
		if(inputTyp.equals("rootinsert")){
			if(title.length()==0) {
				...
				rset= stmt.executeQuery("select *from gongji where id=1;");
				rset.next();				
			}
			else{
				rset= stmt.executeQuery("select ifnull(max(id),0) from gongji;");
				if(rset.next())
					rootid =rset.getInt(1)+1;
				stmt.execute("insert into gongji(title, date, content, rootid, nodeid,viewRank,viewcnt) values('"+title+"','"+Tdate+"','"+content+"',"+rootid+",0,0,0);");
				rset= stmt.executeQuery("select *from gongji where rootid="+rootid+" and nodeid="+nodeid+" ;");
				rset.next();
		...
		if(inputTyp.equals("nodeinsert")){
			if(title.length()==0) {
				...
				int maxid=0;
				out.println("123");
				stmt.execute("insert into gongji(title, date, content, rootid, nodeid,viewRank,viewcnt) values('"+title+"','"+Tdate+"','"+content+"',"+rootid+","+nodeid+","+viewRank+",0);");
				out.println("456");
				rset= stmt.executeQuery("select ifnull(max(id),0) from gongji;");
				rset.next();
				maxid = rset.getInt(1);
				stmt.execute("update gongji set viewRank=(viewRank+1) where rootid="+rootid+" and id != "+maxid+" and viewRank>= "+viewRank+";");
				out.println("789");
		...
		if(inputTyp.equals("update")){
			if(title.length()==0) {
				...
				stmt.execute("update gongji set title='"+title+"', date='"+Tdate+"', content='"+content+"' where id ="+id+";");
...
<%
		if(inputTyp.equals("delete")){			
			int nextRank=0;
			rset= stmt.executeQuery("select viewRank from gongji where rootid ="+rootid+" and nodeid = "+nodeid+" and  viewRank > "+viewRank+";");
			if(rset.next()){
				nextRank = rset.getInt(1); 
				stmt.execute("delete from gongji where rootid="+rootid+" and viewRank >= "+viewRank+" and viewRank < "+nextRank+";");
			}
			else
				stmt.execute("delete from gongji where rootid="+rootid+" and viewRank >= "+viewRank+";");
